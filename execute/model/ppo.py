...
def transform_buffer(buff):
    ...
    # Tính toán reward trung bình và tốc độ trung bình
    avg_reward = np.mean(np.mean(r_batch, axis=1))
    avg_speed = np.mean(speed_batch, axis=0)

    # Ghi nhật ký kết quả huấn luyện
    # logger_ppo.info(f"Average Reward: {avg_reward}, Average Speed: {np.mean(avg_speed,axis=0)}, Avg_speed_each: {avg_speed}")
...
def ppo_update_stage1(policy, optimizer, batch_size, memory, epoch,
    ...
    for update in range(epoch):
        sampler = BatchSampler(SubsetRandomSampler(list(range(advs.shape[0]))), batch_size=16,
                               drop_last=False)
