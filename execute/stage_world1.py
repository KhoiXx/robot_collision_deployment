...
class StageWorld():
    ...
    def get_reward_and_terminate(self, t):
        ...
        if t > 150:
            terminate = True
            result = 'Time out'
        reward = reward_g + reward_c + reward_w + reward_p
        return reward, terminate, result


    # def get_reward_and_terminate(self, t):
    #     terminate = False
    #     decay_factor = np.exp(-0.01 * t)  # Exponential decay with lambda = 0.01 (adjust as needed)
    ...
    #     self.pre_distance = copy.deepcopy(self.distance)
    #     self.distance = np.sqrt((self.goal_point[0] - x) ** 2 + (self.goal_point[1] - y) ** 2)
    #     self.move_distance = np.sqrt((x - self.pre_pos[0])**2 + (y - self.pre_pos[1])**2)
    #     self.pre_pos = [x, y]

    #     # Reward components
    #     reward_g = (self.pre_distance - self.distance) * 3.0  * decay_factor
    #     reward_c = 0
    #     reward_w = 0
    #     reward_s = 0
    #     if self.distance > 3 * self.goal_size:
    #         reward_s = v**2 * 0.2  # Reward for moving faster (encourage speed)
    #     else:
    #         reward_s = -0.5 * (v - 0.2) ** 2  # Phạt khi di chuyển quá nhanh khi gần mục tiêu

    #     reward_p = 0  # Penalty for proximity to obstacles

    #     # Check laser scan readings to penalize being too close to obstacles
    #     start_index = (len(laser_scan) - 200) // 2
    #     end_index = start_index + 200
    #     # Chọn 200 phần tử ở giữa
    #     laser_scan = laser_scan[start_index:end_index]
    #     laser_scan = medfilt(laser_scan, 3)
    #     min_distance_to_obstacle = np.min(laser_scan)
    #     if min_distance_to_obstacle < 0.4 and v > 0.8:
    #         reward_p = -1.1 * (v - 0.8)  * (0.4 - min_distance_to_obstacle)  # Strong penalty for being too close to obstacles
    ...
    #     if self.distance < self.goal_size:
    #         terminate = True
    #         reward_g = 20  # Increased goal reward to make it more desirable
    #         result = 'Reach Goal'

    #     # Collision penalty
    #     if is_crash == 1:
    #         terminate = True
    #         reward_c = -20.0  # Increased penalty for crashing
    #         result = 'Crashed'

    #     # Angular velocity penalty to discourage sharp turns at high speed
    #     if np.abs(w) > 1.5:
    #         reward_w = -0.1 * np.abs(w)  # Stronger penalty for excessive turning

    #     # Timeout penalty
    #     if t > 260:
    #         terminate = True
    #         result = 'Time out'
    #         reward_c -= 10  # Penalty for timing out

    #     # Combine all rewards and penalties
    #     reward = reward_g + reward_c + reward_w + reward_s + reward_p
