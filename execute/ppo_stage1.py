...
def run(comm, env, policy, policy_path, action_bound, optimizer):
    ...
    for id in range(MAX_EPISODES):
        ...
        while not terminal and not rospy.is_shutdown():
            ...
            # rate.sleep()
            rospy.sleep(0.001)  
            # print(f"Env: {env.index}, Real action: {real_action}")
            speed_arr.append(real_action)
            move_dis += env.move_distance
            ...
            step += 1
            state = state_next
            # Moi thang tu luu reward va speed cua no trong mot mang, khi goi tinh trung binh tren tung episode thi goi ra gui len root bawgn send va recv de tinh
            logger_env.info(f"{id+1},{r:.5f},{env.move_distance:.5f},{real_action[0]:.4f},{real_action[1]:.4f},{env.distance},{terminal},{result},{(time.time() - start_time):.5f}")

        if env.index == 0:
            if global_update != 0 and global_update % 20 == 0:
                torch.save(policy.state_dict(), policy_path + '/stage1_fix.pth')
                logger.info('########################## model saved when update {} times#########'
                            '################'.format(global_update))
        distance = np.sqrt((env.goal_point[0] - env.init_pose[0])**2 + (env.goal_point[1]-env.init_pose[1])**2)

        logger.info('Env %02d, Goal (%05.1f, %05.1f), Episode %05d, Step %03d, Reward %-5.1f, Init distance %05.3f, Final distance %05.3f, %s' % \
                    (env.index, env.goal_point[0], env.goal_point[1], id + 1, step, ep_reward, distance, env.distance, result))
        # logger.info(f"Speed: {env.avg_speed / global_step}")
...
if __name__ == '__main__':
    ...
    env = StageWorld(226, index=rank, num_env=NUM_ENV)
    reward = None
    action_bound = [[0, -2], [1.1, 2]]

    logger_env = logging.getLogger(f'log_env_{rank}')
    logger_env.setLevel(logging.INFO)
    log_env_handler = logging.FileHandler(f'./log/log_env_{rank}.log', mode='a')
    log_env_handler.setLevel(logging.INFO)
    logger_env.addHandler(log_env_handler)
    ...
    if rank == 0:
        ...
        file = policy_path + '/stage2_new.pth'
