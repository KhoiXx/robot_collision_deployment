...
\item Fully connected layer 128 nơ-ron + ReLU
\item Output layer: 1 nơ-ron (giá trị estimate)
\end{itemize}

\textbf{Ý nghĩa của output head riêng:} Việc tách riêng output head cho Actor và Critic trong khi chia sẻ encoder có 2 lợi ích: (1) Encoder học được đặc trưng đại diện chung hữu ích cho cả việc ra quyết định (policy) và đánh giá trạng thái (value), tăng hiệu quả học tập; (2) Output heads riêng cho phép mỗi network tối ưu cho mục tiêu khác nhau - Actor tối ưu cho action selection, Critic tối ưu cho value prediction - mà không xung đột gradient.
...
\subsubsection{Dùng các optimizers riêng cho Actor và Critic}
...
\subsection{Các kỹ thuật cải tiến đượi performancec áp dụng}
...
\textbf{Hệ thống cảm biến:} LiDAR 360° được gắn ở trung tâm tầng trên với 455 beams, scan range ~12m, frequency 5-10Hz. LiDAR này tương thích trực tiếp với mô phỏng (cùng 455 tia laser), giúp giảm khác biệt giữa mô phỏng và thực tế. IMU MPU6050 (gyroscope + accelerometer) đo góc xoay và gia tốc, kết hợp với wheel odometry qua UKF để ước lượng pose chính xác.
...
\node[sensor, minimum width=3.5cm] (imu) at (5, -0.3) {IMU MPU6050};
