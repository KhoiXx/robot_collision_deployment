...
Luận văn đã thiết kế và xây dựng thành công 1 robot prototype với chi phí thấp, bao gồm:
...
\item IMU hỗ trợ sensor fusion với UKF
\end{itemize}
...
\subsection{Về triển khai từ mô phỏng vào thực tế}
...
\item \textbf{Các cơ chế an toàn:} Giới hạn vận tốc (0.3 m/s), safety check, stuck detection và recovery đảm bảo robot hoạt động an toàn trong thực nghiệm.
\end{itemize}
...
\item \textbf{LiDAR tần số cao:} Sử dụng LiDAR chất lượng tốt hơn, cho phép tăng tốc độ di chuyển và phản ứng nhanh hơn với vật cản động.
...
\begin{itemize}[nosep]
\item \textbf{Xử lý deadlock:} Nghiên cứu thêm cơ chế phát hiện và giải quyết deadlock khi nhiều robots cùng đi qua một điểm hẹp, có thể kết hợp thêm một số rule định sẵn cho robot để tránh nhau, và cho nó học khi huấn luyện.
\item \textbf{Domain randomization:} Trong quá trình huấn luyện, thay đổi ngẫu nhiên các thông số môi trường (nhiễu cảm biến, độ trễ, ma sát bánh xe) để policy học được cách thích nghi với nhiều điều kiện khác nhau. Điều này giúp model hoạt động tốt hơn khi chuyển từ mô phỏng sang thực tế.
\item \textbf{Hierarchical policy:} Chia policy thành 2 tầng: tầng cao (high-level) quyết định hướng đi tổng thể và tầng thấp (low-level) điều khiển chi tiết để tránh va chạm. Kiến trúc này giúp robot xử lý tốt hơn các tình huống phức tạp như đi qua hành lang hẹp hoặc né nhiều vật cản liên tiếp.
\end{itemize}

\subsection{Mở rộng quy mô thực nghiệm}

\begin{itemize}[nosep]
\item Tăng số lượng robots thực nghiệm lên 5-10 để đánh giá khả năng scale của hệ thống
\item Thử nghiệm trong nhiều môi trường với điều kiện ánh sáng và địa hình đa dạng
...
\item \textbf{Triển khai trên nhiều bài toán khác:} Mở rộng policy để xử lý nhiều task khác nhau (điều hướng trong môi trường không xác định trước, điều khiển tác hợp trong cùng môi trường - cooperative manipulation).
...
\item \textbf{3D environment:} Mở rộng phương pháp cho môi trường 3D với các robots có khả năng bay (drone) hoặc di chuyển trên địa hình phức tạp.
