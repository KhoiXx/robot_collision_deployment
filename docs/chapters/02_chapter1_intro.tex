...
\begin{itemize}
    ...
    \item \textbf{Số lượng:} 44 robots hoạt động đồng thời trong mô phỏng; 1 robot trong thực nghiệm
\end{itemize}
...
\begin{itemize}
    ...
    \begin{itemize}
        \item \textit{Adaptive Learning Rate Scheduler}: Điều chỉnh learning rate động dựa trên performance để thoát khỏi plateau và duy trì momentum khi đang học tốt
        \item \textit{Dense Rewards}: Cải tiến hàm reward với các thành phần chi tiết (timeout penalty, heading guidance, safety margin, smoothness) để hướng dẫn robot học hiệu quả hơn
        \item \textit{Observation Normalization}: Chuẩn hóa input sử dụng running statistics để ổn định quá trình học
        \item \textit{Entropy Decay}: Giảm dần entropy coefficient từ 8×10⁻³ xuống 2×10⁻³ để cân bằng exploration và exploitation
        \item \textit{Value Clipping}: Giảm instability trong quá trình huấn luyện
        \item \textit{Two-stage Training}: Chiến lược huấn luyện hai giai đoạn để cải thiện khả năng mở rộng
    \end{itemize}
