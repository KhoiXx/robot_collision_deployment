...
Thực nghiệm được thực hiện với 17 test cases, chia thành 4 nhóm scenarios với độ khó tăng dần: (1) không có vật cản, (2) vật cản tĩnh, (3) 1 người động, và (4) 2 người động. Bảng \ref{tab:real_robot_summary} tổng hợp kết quả theo từng nhóm.
...
Đây là nhóm test quan trọng nhất để đánh giá khả năng collision avoidance trong môi trường thực tế. Robot đạt \textbf{75\% success rate} (3/4 tests) với 1 người và \textbf{50\%} (1/2 tests) với 2 người.
...
\caption{Robot tránh vật cản động (người): đường xanh dương là trajectory mapping, đường xanh lá là trajectory hiện tại. Hai hình chữ V là chân người được LiDAR phát hiện.}
...
\begin{tabular}{lcc}
\hline
\textbf{Metric} & \textbf{Simulation (Stage 2)} & \textbf{Real Robot} \\
\hline
Success Rate (\%) & 89.18 & 82.4 \\
Timeout Rate (\%) & 1.78 & 17.6 \\
Collision Rate (\%) & 9.04 & 0 \\
Number of Tests & 4480 & 17 \\
Max Velocity (m/s) & 0.7 & 0.3 \\
\hline
\end{tabular}
\end{table}
...
\item \textit{Recovery behavior:} Khi phát hiện stuck, và trong giới hạn an toàn robot tự động lùi lại một đoạn ngắn và thử hướng di chuyển khác
\end{itemize}
\end{itemize}

Tốc độ giảm là trade-off cần thiết để đảm bảo an toàn, bù đắp cho độ trễ mạng và tần số LiDAR thấp (10Hz).
...
\item \textbf{Giải pháp hiện tại:} Sử dụng Cartographer SLAM làm nguồn localization chính. Trong môi trường thực nghiệm (~4m x 3m) với nhiều vật cản, Cartographer hoạt động ổn định và đạt mức chấp nhận được cho collision avoidance.
...
\textbf{Về phần cứng:}
\begin{itemize}[nosep]
\item Nâng cấp Jetson Nano lên Jetson Orin để chạy inference local, giảm latency mạng và tăng tính độc lập của robot.
...
\textbf{Về thuật toán:}
\begin{itemize}[nosep]
\item \textbf{Online learning / Continual learning:} Cho phép robot học thêm từ các tình huống thực tế mà không cần retrain từ đầu. Đây là hướng quan trọng để thu hẹp khoảng cách từ mô phỏng tới thực tế - robot có thể fine-tune policy dựa trên kinh nghiệm thực tế, đặc biệt với các scenarios hiếm gặp trong mô phỏng (người xuất hiện đột ngột, đứng chắn đường, hành lang hẹp). Tuy nhiên, việc triển khai online learning đòi hỏi phần cứng mạnh hơn để chạy cả inference và training trên robot.
