...
def generate_text_report(csv_file, steps, output_prefix):
    """
    Generate comprehensive text report with stability analysis in Vietnamese
    """
    ...
    with open(report_file, 'w', encoding='utf-8') as f:
        ...
        f.write(f"Ngày phân tích: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
        f.write(f"Số lần thử nghiệm bước: {len(steps)}\n\n")

        f.write("=" * 80 + "\n")
        f.write("TÓM TẮT - XÁC MINH ỔN ĐỊNH\n")
        f.write("=" * 80 + "\n\n")
        ...
        for target_speed, step_df in steps:
            ...
            if metrics['settling_time'] is None:
                is_stable = False
                stability_issues.append("Không ổn định trong thời gian thử nghiệm")
            elif metrics['settling_time'] > 3.0:
                is_stable = False
                stability_issues.append(f"Thời gian ổn định quá dài: {metrics['settling_time']:.2f}s")

            # Criterion 2: Overshoot should be reasonable (< 15%)
            if abs(metrics['overshoot_percent']) > 15:
                stability_issues.append(f"Vọt lố cao: {metrics['overshoot_percent']:.1f}%")
            ...
            if metrics['setpoint'] != 0:
                ...
                if ss_error_percent > 5:
                    stability_issues.append(f"Sai số xác lập cao: {ss_error_percent:.1f}%")
        ...
        if all_stable:
            ...
            f.write("  - Sai số xác lập rất nhỏ\n")
            f.write("  - Không có dao động hoặc phân kỳ\n\n")
        else:
            f.write("⚠ KẾT LUẬN: PHÁT HIỆN VẤN ĐỀ ỔN ĐỊNH\n\n")
            f.write("Một số đáp ứng bước cho thấy có vấn đề về ổn định. Xem phân tích chi tiết bên dưới.\n\n")
